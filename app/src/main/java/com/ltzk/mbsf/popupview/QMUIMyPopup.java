...
/**
 * 描述：
 * 作者： on 2020-4-16 11:54
 * 邮箱：499629556@qq.com
 */
public class QMUIMyPopup <T extends QMUIBasePopup> extends QMUIBasePopup<T> {
    ...
    public QMUIMyPopup(Context context, int width, int height) {
        super(context);
        mInitWidth = width;
        mInitHeight = height;
    }
    ...
    public T preferredDirection(@QMUIMyPopup.Direction int preferredDirection) {
    ...
    class ShowInfo {
        ...
        ShowInfo(View parent,Rect rect) {
            //this.anchor = anchor;
            // for muti window
            ...
            widthAuchor = rect.right - rect.left;
            heightAuchor = rect.bottom - rect.top;

            parent.getRootView().getLocationOnScreen(anchorRootLocation);
            //anchorRootLocation[0] = 0;
            //anchorRootLocation[1] = 0;

            anchorLocation[0] = rect.left;
            anchorLocation[1] = rect.top;

            anchorCenter = (rect.right + rect.left) / 2;

            parent.getWindowVisibleDisplayFrame(visibleWindowFrame);
            visibleWindowFrame.top = 0;
        ...
        public int getWidthAuchor() {
            return widthAuchor;
        }

        public int getHeightAuchor() {
            return heightAuchor;
        }
    ...
    public T show(@NonNull View parent,Rect rectAnchor) {
        if (mContentView == null) {
            throw new RuntimeException("you should call view() to set your content view");
        }
        QMUIMyPopup.ShowInfo showInfo = new QMUIMyPopup.ShowInfo(parent,rectAnchor);
        calculateWindowSize(showInfo);
        calculateXY(showInfo);
        adjustShowInfo(showInfo);
        processWindowDecoration(showInfo);
        ...
        showAtLocation(parent, showInfo.getWindowX(), showInfo.getWindowY());
        return (T) this;
    }
     ...
     * TODO 修正
     */
    private void processWindowDecoration(QMUIMyPopup.ShowInfo showInfo) {
        final boolean onTop = showInfo.direction == DIRECTION_TOP;
        showInfo.decorationTop = onTop ? 0 : showInfo.decorationTop / 3;
        showInfo.decorationBottom = onTop ? showInfo.decorationBottom / 3 : 0;
        showInfo.x = showInfo.decorationLeft;
        showInfo.decorationLeft = 0;
        showInfo.decorationRight = 0;
    }

    private void decorateContentView(QMUIMyPopup.ShowInfo showInfo) {
        QMUIMyPopup.ContentView contentView = QMUIMyPopup.ContentView.wrap(mContentView, mInitWidth, mInitHeight);
        ...
        QMUIMyPopup.DecorRootView decorRootView = new QMUIMyPopup.DecorRootView(mContext, showInfo);
        decorRootView.setContentView(contentView);
        mWindow.setContentView(decorRootView);
    }

    private void adjustShowInfo(QMUIMyPopup.ShowInfo showInfo) {
        if (shouldShowShadow()) {
    ...
    private void calculateXY(QMUIMyPopup.ShowInfo showInfo) {
    ...
    private void handleDirection(QMUIMyPopup.ShowInfo showInfo, int currentDirection, int nextDirection) {
        if (currentDirection == DIRECTION_CENTER_IN_SCREEN) {
            ...
            showInfo.y = showInfo.anchorLocation[1] + showInfo.getHeightAuchor() + mOffsetYIfBottom;
    ...
    private void calculateWindowSize(QMUIMyPopup.ShowInfo showInfo) {
    ...
    private void setAnimationStyle(float anchorProportion, @QMUIMyPopup.Direction int direction) {
    ...
    static class ContentView extends QMUIFrameLayout {
        ...
        static QMUIMyPopup.ContentView wrap(View businessView, int width, int height) {
            QMUIMyPopup.ContentView contentView = new QMUIMyPopup.ContentView(businessView.getContext());
    ...
    class DecorRootView extends FrameLayout implements IQMUISkinDispatchInterceptor {
        private QMUIMyPopup.ShowInfo mShowInfo;
        private View mContentView;
        ...
        private DecorRootView(Context context, QMUIMyPopup.ShowInfo showInfo) {
            super(context);
            mShowInfo = showInfo;
            ...
            if (mShowArrow) {
                if (mShowInfo.direction == DIRECTION_TOP) {
                    ...
                    if (!mRemoveBorderWhenShadow || !shouldShowShadow()) {
                        ...
                        canvas.drawLine(0, 0, mArrowWidth / 2 + mBorderWidth / 2, mArrowHeight, mArrowPaint);
                        canvas.drawLine(mArrowWidth / 2 - mBorderWidth / 2, mArrowHeight, mArrowWidth, 0, mArrowPaint);
                    }
                    canvas.restore();
