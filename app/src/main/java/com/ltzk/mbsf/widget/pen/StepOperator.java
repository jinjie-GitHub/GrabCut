...
public class StepOperator {
    ...
    public Bitmap getCurrentBitmap(final Bitmap def) {
        try {
            if (mBitmaps != null && mBitmaps.size() > 0) {
                if (currentIndex < 0) {
                    currentIndex = 0;
                }
                if (currentIndex >= mBitmaps.size()) {
                    currentIndex = mBitmaps.size() - 1;
                }
                if (currentIndex >= 0 && currentIndex < mBitmaps.size()) {
                    return mBitmaps.get(currentIndex);
                }
            }
        } catch (Throwable e) {
