...
import com.spx.library.util.Config.Companion.maxSelection
import com.spx.library.util.Config.Companion.minSelection
import com.spx.library.util.ViewUtil
import kotlinx.android.synthetic.main.activity_video_clip.view.*

/**
 * Created by JinJie on 2021/09/16 视频裁剪条
 */
class ClipContainer : FrameLayout {
    ...
    private val progressBarTouchListener = object : View.OnTouchListener {
        ...
        override fun onTouch(v: View, event: MotionEvent): Boolean {
            when (event.action) {
                MotionEvent.ACTION_DOWN -> downX = (event.x - ViewUtil.dpToPx(20))
                MotionEvent.ACTION_MOVE -> {
                    val xDistance = event.x - downX
                    if (xDistance != 0f) {
                        var newTransX = playProgressBar.translationX + xDistance
                        Log.d(TAG, "onTouch  xDistance:$xDistance, newTransX: $newTransX")
                        adjustProgressBar(playProgressBar, newTransX)
                        onPreviewChange(false)
                        //去掉recyclerView左内边距
                        downX = playProgressBar.translationX - ViewUtil.dpToPx(20)
                    }
                }
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
...
//        Log.d(TAG, "adjustProgressBar  transX_:$transX_, transX: $transX")
    ...
    interface Callback {
        fun onSelectionChang(totalCount: Int, startMillSec: Long, endMillSec: Long, finished: Boolean, isRight: Boolean)
    ...
    fun init(context: Context) {
        ...
        progressPaint = Paint()
        progressPaint!!.color = context.resources.getColor(R.color.frame_bar_color)
        ...
        with(context.resources) {
            recyclerViewPadding = getDimensionPixelSize(R.dimen.clip_frame_bar_width_outer)
            framebarHeight = ViewUtil.dpToPx(5)//上下边框矩形高度
            ...
            minProgressBarX = getDimensionPixelSize(R.dimen.clip_frame_bar_width_outer)
        }
    }
...
//        clipFrameLayout = findViewById(R.id.clipContainer)
        ...
        recyclerView.setOnTouchListener(progressBarTouchListener)
...
//        rightFrameLeft = rightFrameBar.translationX
        maxProgressBarX = width - resources.getDimensionPixelSize(R.dimen.clip_frame_bar_width_outer)
        frameWidth = width - leftFrameBar!!.width - rightFrameBar!!.width
    }
...
//            Log.d(TAG, "onFrameMoved: leftShadowStart:$leftShadowStart, leftShadowEnd:$leftShadowEnd")
            ...
            if (callback != null) {
                if (right != null) {
                    callback!!.onSelectionChang(itemCount, startMillSec.toLong(), endMillSec.toLong(), finished, right)
                }
            }
            invalidate()
            return
